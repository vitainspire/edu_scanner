...
import {
  ...
  AlertCircle, ScanLine, ChevronRight,
} from "lucide-react";
...
function BatchScanInner() {
  ...
  // Load test + class roster on mount
  useEffect(() => {
    ...
    try {
      ...
      const res = await fetch("/api/grade-scan", {
        ...
        body: JSON.stringify({
          ...
          totalMarks: testInfo?.total_marks ?? 0,
      ...
      setResult({ matchedStudent: data.matchedStudent, score: data.score, previewUrl });
  ...
  async function handleSave() {
    ...
    if (scoreInput.trim() === "" || isNaN(score) || score < 0 || score > (testInfo?.total_marks ?? 0)) {
      setError(`Enter a valid score (0 – ${testInfo?.total_marks ?? 0}).`);
      return;
    }
  ...
  const scorePct = totalMarks > 0 && scoreInput !== "" && !isNaN(scoreNum)
    ? Math.min(100, Math.round((scoreNum / totalMarks) * 100))
    : 0;
  const scoreColor =
    scorePct >= 70 ? "text-emerald-600" : scorePct >= 40 ? "text-amber-600" : "text-red-500";

  return (
    <div
      className="flex flex-col"
      style={{ minHeight: "calc(100dvh - 3.5rem - env(safe-area-inset-top, 0px))" }}
    >
      {/* ── Header ──────────────────────────────────────────────── */}
      <div
      ...
      {stage === "capture" && (
        <div className="flex-1 flex flex-col items-center justify-between gap-5 pb-safe">
          {/* Viewfinder */}
          <div
            className="w-full flex-1 rounded-3xl overflow-hidden flex flex-col items-center justify-center gap-5"
            style={{
            ...
            {scannedCount > 0 ? (
              ...
              <div className="relative w-36 h-48">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
                  <ScanLine size={26} className="text-indigo-400/60" />
                  <p className="text-xs text-indigo-300/50 text-center px-2">Align answer sheet</p>
                </div>
              </div>
            )}
          </div>
          ...
          >
            <Camera size={24} />
            {testInfo ? "Take Photo" : "Loading…"}
          </button>

          {scannedCount > 0 && (
            <button
              onClick={() => router.push(`/dashboard/${classId}/tests/${testId}/scan`)}
              className="w-full min-h-[48px] border-2 border-gray-200 text-gray-600 font-bold text-base rounded-2xl active:scale-[0.97] transition-transform flex items-center justify-center gap-2 bg-white"
            >
              View Progress
      ...
      {stage === "grading" && (
        <div className="flex-1 flex flex-col items-center justify-center gap-6 pb-safe">
          <div className="w-full rounded-3xl overflow-hidden bg-gray-900 flex items-center justify-center"
            ...
            <div className="flex flex-col items-center gap-4 py-10">
              ...
              <div className="text-center">
                <p className="font-black text-white text-lg">Reading name &amp; grading…</p>
                <p className="text-xs text-white/40 mt-1.5">Please keep the screen on</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {(stage === "confirm" || stage === "saving") && result && (
        <div className="flex-1 flex flex-col gap-4 pb-safe">
          ...
          {/* Thumbnail */}
          <div
            className="w-full rounded-3xl overflow-hidden bg-gray-900 flex items-center justify-center shrink-0"
            style={{ minHeight: "min(45vw, 32vh)" }}
          >
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            {result.matchedStudent ? (
              <div className="flex items-center gap-3 bg-emerald-50 px-4 py-3 border-b border-emerald-100">
                ...
                <div>
                  <p className="text-[10px] font-black tracking-[0.2em] uppercase text-emerald-600">AI matched</p>
                  <p className="text-sm font-black text-gray-900 mt-0.5">{result.matchedStudent.name}</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 bg-amber-50 px-4 py-3 border-b border-amber-100">
                ...
                <div>
                  <p className="text-[10px] font-black tracking-[0.2em] uppercase text-amber-600">Name not found</p>
                  <p className="text-xs text-gray-500 mt-0.5">Select the student manually</p>
                </div>
              </div>
            )}
            ...
            <div className="px-4 py-4 space-y-4">
              {/* Student selector */}
              <div>
                <label className="block text-[10px] font-black tracking-[0.2em] uppercase text-gray-400 mb-1.5">
                  Student
                </label>
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none px-4 py-3 pr-8 rounded-xl border border-gray-200 bg-gray-50 text-sm font-bold text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent min-h-[44px]"
                  >
                    <option value="">— Select student —</option>
                    {students.map((s) => (
                      <option key={s.id} value={s.id}>
                        {String(s.roll_number).padStart(2, "0")} · {s.name}
                      </option>
                    ))}
                  </select>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="0"
                  className="w-full text-4xl font-black text-center text-gray-900 bg-gray-50 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent py-3"
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 min-h-[56px] border-2 border-gray-200 text-gray-700 font-bold text-base rounded-2xl active:scale-95 transition-transform flex items-center justify-center gap-2 bg-white disabled:opacity-50"
            >
              <RefreshCw size={17} />
              Retake
            </button>
