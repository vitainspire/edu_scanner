...
export default async function ScanPage({ params }: Props) {
  ...
  const [classResult, testResult, studentsResult, marksResult] =
    await Promise.all([
      supabase
        .from("classes")
      ...
      supabase
        ...
        .eq("is_active", true)
        .order("roll_number"),
      // Marks absence is expected (no scans yet) — not a hard failure.
      supabase
        .from("marks")
        .select("student_id, score, source")
        .eq("test_id", testId),
    ]);
  ...
  const scoreMap = new Map<string, number>(
    marks.map((m) => [m.student_id, m.score])
  );
